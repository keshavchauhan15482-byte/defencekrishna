...
for (let i = 0; i < mutationSeeds.length; i++) {
  ...
  for (const entry of learned.newlyLearned || []) {
    maxPerTokenCandidates = Math.max(maxPerTokenCandidates, Number(entry.mutationValidation && entry.mutationValidation.candidateCount || 0));
...
// production accuracy claims; they prevent obvious defensive regressions.
...
assert.ok(metrics.mutations.maxPerTokenCandidates <= metrics.mutations.configuredPerTokenBudget, 'per-token mutation budget exceeded');
