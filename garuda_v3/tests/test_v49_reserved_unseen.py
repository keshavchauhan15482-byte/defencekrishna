...
try:
    import numpy as np
    from garuda_v3.v49_reserved_unseen import (
        DEV_FAMILIES,
        RESERVE_FAMILIES,
        WORLD_CANDIDATES,
        HYBRID_CANDIDATES,
...
@unittest.skipUnless(V49_DEPS, 'V49 research dependencies are intentionally separate')
class V49ReservedUnseenTests(unittest.TestCase):
    ...
    def test_candidate_selection_prioritizes_fpr_budget(self):
        names = ('a', 'b')
        dev = {
            'fam1': {'seeds': {
                '42': {'status': 'evaluated', 'scores': {'a': {'test': {'recall': .70, 'fpr': .005}}, 'b': {'test': {'recall': .95, 'fpr': .03}}}},
                '43': {'status': 'evaluated', 'scores': {'a': {'test': {'recall': .72, 'fpr': .005}}, 'b': {'test': {'recall': .95, 'fpr': .03}}}},
            }},
            'fam2': {'seeds': {
                '42': {'status': 'evaluated', 'scores': {'a': {'test': {'recall': .65, 'fpr': .006}}, 'b': {'test': {'recall': .99, 'fpr': .02}}}},
                '43': {'status': 'evaluated', 'scores': {'a': {'test': {'recall': .66, 'fpr': .006}}, 'b': {'test': {'recall': .99, 'fpr': .02}}}},
            }},
        }
    ...
    def test_candidate_groups_do_not_overlap(self):
