...
def build_report(graphs, split_manifest, predictions_path, metrics_path, architecture):
    ...
    splits, _ = campaign_examples(datasets, manifest, 8, 4, 2, allow_unknown=True)
    train, valid, test = [batch(datasets, split, 8, 4) for split in splits]
    ...
    return {
        ...
        "campaign_bootstrap_all_known": campaign_bootstrap(
            ...
            probabilities,
            threshold,
            seed=int(training_report.get("config", {}).get("seed", 42)),
        ),
