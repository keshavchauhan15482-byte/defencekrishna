"""V48 reserved unseen-family scoring study for X-IIoTID.

V47 exposed five family-disjoint diagnostics.  V48 uses those five families only as
...
def empirical_surprise(reference, values):
    """Map a high-valued signal to benign empirical tail surprise.

    Calibration benign samples define the reference CDF.  Returned values are
    -log10 empirical upper-tail probabilities, so disparate signals can be fused
    without using attack positives for calibration.
    """
...
def evaluate_scores(scores, split, policy_benign):
    ...
    for name, score in scores.items():
        ...
        rows[name] = {
            "threshold": threshold,
            "test": binary_metrics(y_eval, score[eval_idx], threshold),
        }
    return rows
...
def choose_candidate(dev_results, names):
    ...
    # Frozen lexicographic rule: first maximize complete gates, then the number of
    # families respecting the FPR budget, then recall while under budget, then
    # overall recall, and finally prefer lower average FPR. Candidate name is only
...
def compact_reserve(result, selected_world, selected_hybrid):
    out = {
        ...
        "state_gate_passed_all_seeds": True,
        "scores": {},
    }
...
def main():
    ...
    if out.exists():
        parser.error("Output already exists; V48 evidence is immutable")
    ...
    for family_name in DEV_FAMILIES:
        print(f"V48 development pseudo-unseen family={family_name}", flush=True)
        dev_results[family_name] = run_family(
            sequences, time_masks, family_name, tuple(args.seeds), args.epochs, ALL_CANDIDATES
        )
    ...
    for family_name in RESERVE_FAMILIES:
        print(f"V48 RESERVED family={family_name}", flush=True)
        reserve_results[family_name] = run_family(
            sequences, time_masks, family_name, tuple(args.seeds), args.epochs, frozen
        )

    reserve_summary = {
        family_name: compact_reserve(result, selected_world, selected_hybrid)
        for family_name, result in reserve_results.items()
    }
    report = {
        "protocol": "V48 frozen-score reserved unseen-family evaluation",
        ...
        "reserved_unscored_before_v48_families": list(RESERVE_FAMILIES),
